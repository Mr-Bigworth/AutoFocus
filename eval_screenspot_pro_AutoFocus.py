...
def parse_args():
    ...
    parser.add_argument('--model_type', type=str, default='qwen25vl_AutoFocus_72b_PPL', required=True)
...
def build_model(args):
    ...
    if model_type == "qwen25vl_AutoFocus_72b_PPL":
        from models.qwen25vl_AutoFocus_72b_PPL import Qwen25VLModel
        model = Qwen25VLModel(model_name = model_name_or_path)
    elif model_type == "qwen25vl_AutoFocus_72b_PPL_circle":
        from models.qwen25vl_AutoFocus_72b_PPL_circle import Qwen25VLModel
        model = Qwen25VLModel(model_name = model_name_or_path)
    elif model_type == "qwen25vl_AutoFocus_72b_PPL_square":
        from models.qwen25vl_AutoFocus_72b_PPL_square import Qwen25VLModel
        model = Qwen25VLModel(model_name = model_name_or_path)
    else:
    ...
    for i, sample in enumerate(tqdm(tasks_to_run)):
        ...
        try:
            if sample["gt_type"] == "positive":
                response = model.ground_with_AutoFocus(instruction=sample["prompt_to_evaluate"], image=img_path, task_id=filename.replace('/', '_').replace('.png', ''), debug=args.debug, ground_truth=sample["bbox"])
        except Exception as e:
        ...
        if "regionfocus_applied" in response:
            sample_result.update({"regionfocus_applied": response["regionfocus_applied"]})
        if "self_judgement" in response:
            sample_result.update({"self_judgement": response["self_judgement"]})
        if "perplexity" in response:
            sample_result.update({"perplexity": response["perplexity"]})
        if "perplexity_content" in response:
            sample_result.update({"perplexity_content": response["perplexity_content"]})
        if "perplexity_x" in response :
            sample_result.update({"perplexity_x": response["perplexity_x"]})
        if "perplexity_y" in response :
            sample_result.update({"perplexity_y": response["perplexity_y"]})
        if "self_judgement_prob_cw" in response:
            sample_result.update({"self_judgement_prob_cw": response["self_judgement_prob_cw"]})
        if sample["gt_type"] == "positive":
