...
def plot_points_on_image(image, points, colors=None, sizes=None, markers=None, labels=None, save_path=None):
    ...
    if markers is None:
        markers = ['circle' for _ in range(len(points))]
    ...
    for i, (x, y) in enumerate(points):
        ...
        marker = markers[i] if i < len(markers) else 'circle'
...
class Qwen25VLModel:
    ...
    def judge_inference(self, instruction, image, point, debug=False, task_id=None, system_message=None):
        ...
        judge_prompt = (
            f'Given the instruction: "{instruction}", I highlighted a pink circle on the image, '
            f'Is this pink circle position correct and precise for the instruction? '
